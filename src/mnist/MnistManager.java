...
public class MnistManager {
     ...
     * arguments in case reading only one of the files (images and labels) is required.
    ...
    public MnistManager(String imagesFile, String labelsFile, boolean train) throws IOException {
        if (imagesFile != null) {
            ...
            if(train) imagesArr = images.readImagesUnsafe(NUM_EXAMPLES);
            else imagesArr = images.readImagesUnsafe(NUM_EXAMPLES_TEST);
        }
        if (labelsFile != null) {
            labels = new MnistLabelFile(labelsFile, "r");
            if(train) labelsArr = labels.readLabels(NUM_EXAMPLES);
            else labelsArr = labels.readLabels(NUM_EXAMPLES_TEST);
        }
        System.out.println();
    }
